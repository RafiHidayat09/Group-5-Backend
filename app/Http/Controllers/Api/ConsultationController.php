<?php

namespace App\Http\Controllers\Api;

use App\Http\Controllers\Controller;
use App\Models\Consultation;
use App\Models\Psychologist;
use App\Models\Wallet;
use App\Models\User;
use App\Models\WalletTransaction;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class ConsultationController extends Controller
{
    /**
     * Helper function to get full avatar URL
     */
    private function getAvatarUrl($path)
    {
        if (!$path) return null;
        if (str_starts_with($path, 'http')) {
            return $path;
        }
        return asset('storage/' . $path);
    }

    /**
     * Start a new consultation
     */
    public function start(Request $request)
    {
        $request->validate([
            'psychologist_id' => 'required|exists:users,id'
        ]);

        DB::beginTransaction();

        try {
            $patient = auth()->guard('api')->user();

            // 1. Cari Profil Psikolog
            $psychologistProfile = Psychologist::where('user_id', $request->psychologist_id)->first();

            if (!$psychologistProfile) {
                return response()->json(['success' => false, 'message' => 'User ini bukan psikolog'], 404);
            }

            if ($patient->id === $psychologistProfile->user_id) {
                return response()->json(['success' => false, 'message' => 'Tidak bisa konsultasi dengan diri sendiri'], 400);
            }

            // 2. Cek Ketersediaan
            if (!$psychologistProfile->is_available || $psychologistProfile->status !== 'online') {
                return response()->json(['success' => false, 'message' => 'Psikolog sedang offline atau tidak tersedia.'], 400);
            }

            // 3. Cek Konsultasi Aktif
            $activeConsultation = Consultation::where('user_id', $patient->id)
                ->where('psychologist_id', $psychologistProfile->id)
                ->whereIn('status', ['pending', 'active'])
                ->first();

            if ($activeConsultation) {
                return response()->json([
                    'success' => true,
                    'consultation_id' => $activeConsultation->id,
                    'requires_payment' => $activeConsultation->status === 'pending',
                    'message' => 'Melanjutkan sesi konsultasi yang ada'
                ]);
            }

            // 4. Buat Konsultasi Baru
            $consultation = Consultation::create([
                'user_id' => $patient->id,
                'psychologist_id' => $psychologistProfile->id,
                'fee' => $psychologistProfile->fee,
                'status' => 'pending'
            ]);

            // 5. Cek Saldo & Auto Pay (HANYA POTONG SALDO PASIEN)
            $wallet = Wallet::where('user_id', $patient->id)->first();
            $requiresPayment = true;

            if ($wallet && $wallet->balance >= $psychologistProfile->fee) {
                // Potong saldo pasien
                $wallet->decrement('balance', $psychologistProfile->fee);

                // Catat Transaksi Keluar (Pasien)
                WalletTransaction::create([
                    'wallet_id' => $wallet->id,
                    'user_id' => $patient->id,
                    'consultation_id' => $consultation->id,
                    'type' => 'payment',
                    'amount' => $psychologistProfile->fee,
                    'description' => 'Konsultasi dengan ' . ($psychologistProfile->user->name ?? 'Psikolog'),
                    'status' => 'completed',
                    'payment_method' => 'wallet'
                ]);

                $consultation->update([
                    'status' => 'active',
                    'started_at' => now()
                ]);

                $requiresPayment = false;
            }

            DB::commit();

            return response()->json([
                'success' => true,
                'consultation_id' => $consultation->id,
                'requires_payment' => $requiresPayment,
                'fee' => $psychologistProfile->fee,
                'wallet_balance' => $wallet?->balance ?? 0,
                'message' => $requiresPayment ? 'Silakan lakukan pembayaran' : 'Konsultasi dimulai'
            ]);

        } catch (\Exception $e) {
            DB::rollBack();
            return response()->json(['success' => false, 'message' => 'Gagal memulai konsultasi', 'error' => $e->getMessage()], 500);
        }
    }

    /**
     * Start consultation session (for psychologist)
     */
    public function startSession($id)
    {
        DB::beginTransaction();
        try {
            $psychologist = auth()->guard('api')->user();

            // Validasi kepemilikan
            $consultation = Consultation::where('psychologist_id', $psychologist->id) // Perlu dicek apakah field ini ID User atau ID Profile
                ->orWhereHas('psychologist', function($q) use ($psychologist) {
                    $q->where('user_id', $psychologist->id);
                })
                ->where('id', $id)
                ->firstOrFail();

            if($consultation->status === 'pending') {
                 $consultation->update([
                    'status' => 'active',
                    'started_at' => now()
                ]);
            }

            DB::commit();
            return response()->json(['success' => true, 'message' => 'Sesi dimulai', 'data' => $consultation]);
        } catch (\Exception $e) {
            DB::rollBack();
            return response()->json(['success' => false, 'message' => 'Gagal memulai sesi', 'error' => $e->getMessage()], 500);
        }
    }

    /**
     * End consultation session AND TRANSFER MONEY
     */
    public function endSession($id)
    {
        DB::beginTransaction();
        try {
            $currentUser = auth()->guard('api')->user();

            // Cari konsultasi (bisa diakhiri oleh Pasien atau Dokter)
            $consultation = Consultation::with('psychologist')
                ->where('id', $id)
                ->where(function($query) use ($currentUser) {
                    $query->where('user_id', $currentUser->id)
                          ->orWhereHas('psychologist', function($q) use ($currentUser) {
                              $q->where('user_id', $currentUser->id);
                          });
                })
                ->where('status', 'active') // Pastikan hanya yang aktif yang bisa diakhiri
                ->firstOrFail();

            // 1. Update Status Konsultasi
            // Kita pakai update manual agar timestamp sinkron dengan transaksi
            $consultation->update([
                'status' => 'ended',
                'ended_at' => now()
            ]);

            // ==========================================================
            // LOGIKA PENCARIAN DANA KE PSIKOLOG (BARU)
            // ==========================================================

            // Hitung Nominal
            $amount = $consultation->fee;
            $adminFee = $amount * 0.1; // Potongan Admin 10%
            $doctorIncome = $amount - $adminFee; // Yang diterima dokter

            // Ambil ID User milik Psikolog
            // (Kita butuh ID User, bukan ID Profile Psikolog, karena Wallet terikat ke User)
            $psychologistUserId = $consultation->psychologist->user_id;
            $patientName = $consultation->user->name ?? 'Pasien';

            // Cari atau Buat Wallet untuk Psikolog
            $doctorWallet = Wallet::firstOrCreate(
                ['user_id' => $psychologistUserId],
                ['balance' => 0]
            );

            // Tambah Saldo Psikolog
            $doctorWallet->increment('balance', $doctorIncome);

            // Catat Transaksi Masuk (Income/Topup)
            WalletTransaction::create([
                'wallet_id' => $doctorWallet->id,
                'user_id' => $psychologistUserId,
                'consultation_id' => $consultation->id,
                'type' => 'topup',
                'amount' => $doctorIncome,
                'description' => 'Pendapatan konsultasi dari ' . $patientName,
                'status' => 'completed',
                'payment_method' => 'system',
                'metadata' => json_encode([
                    'source' => 'consultation',
                    'client_id' => $consultation->user_id,
                    'platform_fee' => $adminFee
                ])
            ]);
            // ==========================================================

            DB::commit();

            return response()->json([
                'success' => true,
                'message' => 'Konsultasi berakhir dan dana diteruskan ke psikolog',
                'data' => [
                    'id' => $consultation->id,
                    'status' => $consultation->status,
                    'ended_at' => $consultation->ended_at
                ]
            ]);

        } catch (\Illuminate\Database\Eloquent\ModelNotFoundException $e) {
            DB::rollBack();
            return response()->json(['success' => false, 'message' => 'Konsultasi tidak ditemukan atau sudah berakhir'], 404);
        } catch (\Exception $e) {
            DB::rollBack();
            return response()->json(['success' => false, 'message' => 'Gagal mengakhiri konsultasi', 'error' => $e->getMessage()], 500);
        }
    }

    /**
     * Pay for consultation (Manual Trigger)
     */
    public function pay(Request $request, $id)
    {
        DB::beginTransaction();
        try {
            $user = auth()->guard('api')->user();

            $consultation = Consultation::where('user_id', $user->id)
                ->where('id', $id)
                ->where('status', 'pending')
                ->firstOrFail();

            $wallet = Wallet::where('user_id', $user->id)->firstOrFail();

            if ($wallet->balance < $consultation->fee) {
                return response()->json(['success' => false, 'message' => 'Saldo tidak cukup'], 400);
            }

            // Nama Dokter
            $doctorName = $consultation->psychologist->user->name ?? 'Psikiater';

            // Potong Saldo Pasien
            $wallet->decrement('balance', $consultation->fee);

            // Catat Transaksi Pasien
            $wallet->transactions()->create([
                'user_id' => $user->id,
                'consultation_id' => $consultation->id,
                'type' => 'payment',
                'amount' => $consultation->fee,
                'description' => 'Pembayaran konsultasi dengan ' . $doctorName,
                'status' => 'completed',
                'payment_method' => 'wallet'
            ]);

            // Update Konsultasi
            $consultation->update([
                'status' => 'active',
                'started_at' => now(),
            ]);

            DB::commit();

            return response()->json([
                'success' => true,
                'message' => 'Pembayaran berhasil',
                'data' => [
                    'consultation_id' => $consultation->id,
                    'status' => $consultation->status,
                    'wallet_balance' => $wallet->balance
                ]
            ]);

        } catch (\Exception $e) {
            DB::rollBack();
            return response()->json(['success' => false, 'message' => 'Gagal melakukan pembayaran', 'error' => $e->getMessage()], 500);
        }
    }

    /**
     * Get consultation detail (Show)
     */
    public function show($id)
    {
        try {
            $currentUser = auth()->guard('api')->user();

            $consultation = Consultation::with(['psychologist.user', 'user'])
                ->where('id', $id)
                ->where(function($query) use ($currentUser) {
                    $query->where('user_id', $currentUser->id)
                          ->orWhereHas('psychologist', function($q) use ($currentUser) {
                              $q->where('user_id', $currentUser->id);
                          });
                })
                ->firstOrFail();

            $psyProfile = $consultation->psychologist;
            $psyUser = $psyProfile->user;

            $data = [
                'consultation_id' => $consultation->id,
                'status' => $consultation->status,
                'fee' => $consultation->fee,
                'started_at' => $consultation->started_at,
                'ended_at' => $consultation->ended_at,
                'rating' => $consultation->rating,
                'review' => $consultation->review,
                'rated' => $consultation->rated,
                'requires_payment' => $consultation->status === 'pending',
                'psychologist' => [
                    'id' => $psyUser->id,
                    'profile_id' => $psyProfile->id,
                    'name' => $psyUser->name,
                    'avatar' => $this->getAvatarUrl($psyUser->avatar),
                    'specialization' => $psyProfile->specialization,
                    'status' => $psyProfile->status,
                    'rating' => $psyProfile->rating,
                    'review_count' => $psyProfile->review_count,
                ],
                'user' => [
                    'id' => $consultation->user->id,
                    'name' => $consultation->user->name,
                    'avatar' => $this->getAvatarUrl($consultation->user->avatar),
                    'email' => $consultation->user->email,
                ]
            ];

            return response()->json(['success' => true, 'data' => $data]);

        } catch (\Exception $e) {
            return response()->json(['success' => false, 'message' => 'Konsultasi tidak ditemukan', 'error' => $e->getMessage()], 404);
        }
    }

    /**
     * Rate consultation
     */
    public function rate(Request $request, $id)
    {
        $request->validate([
            'rating' => 'required|integer|min:1|max:5',
            'review' => 'nullable|string|max:500'
        ]);

        DB::beginTransaction();
        try {
            $user = auth()->guard('api')->user();

            $consultation = Consultation::where('user_id', $user->id)
                ->where('id', $id)
                ->where('status', 'ended')
                ->where('rated', false)
                ->firstOrFail();

            $consultation->update([
                'rating' => $request->rating,
                'review' => $request->review,
                'rated' => true
            ]);

            // Update Rating Dokter
            $psychologist = $consultation->psychologist;

            // Fix division by zero potential
            $currentCount = $psychologist->review_count ?? 0;
            $currentRating = $psychologist->rating ?? 0;

            $oldTotal = $currentRating * $currentCount;
            $newCount = $currentCount + 1;
            $newRating = ($oldTotal + $request->rating) / $newCount;

            $psychologist->update([
                'rating' => round($newRating, 1),
                'review_count' => $newCount
            ]);

            DB::commit();

            return response()->json([
                'success' => true,
                'message' => 'Terima kasih atas penilaian Anda',
                'data' => ['rating' => $request->rating, 'review' => $request->review]
            ]);

        } catch (\Exception $e) {
            DB::rollBack();
            return response()->json(['success' => false, 'message' => 'Gagal mengirim rating', 'error' => $e->getMessage()], 500);
        }
    }

    /**
     * Archive consultation
     */
    public function archive($id)
    {
        try {
            $currentUser = auth()->guard('api')->user();

            $consultation = Consultation::where('id', $id)
                ->where(function($query) use ($currentUser) {
                    $query->where('user_id', $currentUser->id)
                          ->orWhereHas('psychologist', function($q) use ($currentUser) {
                              $q->where('user_id', $currentUser->id);
                          });
                })
                ->firstOrFail();

            $consultation->update(['is_archived' => true]);

            return response()->json([
                'success' => true,
                'message' => 'Konsultasi berhasil diarsipkan'
            ]);

        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
                'message' => 'Gagal mengarsipkan konsultasi',
                'error' => $e->getMessage()
            ], 500);
        }
    }

    /**
     * Unarchive consultation
     */
    public function unarchive($id)
    {
        try {
            $currentUser = auth()->guard('api')->user();

            $consultation = Consultation::where('id', $id)
                ->where(function($query) use ($currentUser) {
                    $query->where('user_id', $currentUser->id)
                          ->orWhereHas('psychologist', function($q) use ($currentUser) {
                              $q->where('user_id', $currentUser->id);
                          });
                })
                ->firstOrFail();

            $consultation->update(['is_archived' => false]);

            return response()->json([
                'success' => true,
                'message' => 'Konsultasi berhasil dikeluarkan dari arsip'
            ]);

        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
                'message' => 'Gagal mengeluarkan konsultasi dari arsip',
                'error' => $e->getMessage()
            ], 500);
        }
    }

    /**
     * Get user's consultations (for both user and psychologist)
     */
    public function userConsultations(Request $request)
    {
        try {
            $user = auth()->guard('api')->user();
            $filter = $request->get('filter', 'all');

            // Tentukan apakah user adalah psikolog atau pasien
            $isPsychologist = Psychologist::where('user_id', $user->id)->exists();

            if ($isPsychologist) {
                // Psikolog: lihat konsultasi dimana dia sebagai psikolog
                $query = Consultation::where('psychologist_id', $user->id)
                    ->orWhereHas('psychologist', function($q) use ($user) {
                        $q->where('user_id', $user->id);
                    })
                    ->with('user:id,name,email,avatar');
            } else {
                // User biasa: lihat konsultasi dimana dia sebagai pasien
                $query = Consultation::where('user_id', $user->id)
                    ->with('psychologist.user:id,name,email,avatar');
            }

            // Filter by status
            if ($filter !== 'all') {
                $query->where('status', $filter);
            }

            $consultations = $query->orderBy('created_at', 'desc')
                ->get()
                ->map(function($consultation) use ($isPsychologist) {
                    if ($isPsychologist) {
                        // Untuk psikolog: tampilkan data pasien
                        return [
                            'id' => $consultation->id,
                            'user_name' => $consultation->user->name,
                            'user_email' => $consultation->user->email,
                            'user_avatar' => $this->getAvatarUrl($consultation->user->avatar),
                            'status' => $consultation->status,
                            'fee' => $consultation->fee,
                            'rating' => $consultation->rating,
                            'review' => $consultation->review,
                            'created_at' => $consultation->created_at,
                            'started_at' => $consultation->started_at,
                            'ended_at' => $consultation->ended_at,
                        ];
                    } else {
                        // Untuk user: tampilkan data psikolog
                        return [
                            'id' => $consultation->id,
                            'psychologist_name' => $consultation->psychologist->user->name,
                            'psychologist_avatar' => $this->getAvatarUrl($consultation->psychologist->user->avatar),
                            'psychologist_specialization' => $consultation->psychologist->specialization,
                            'status' => $consultation->status,
                            'fee' => $consultation->fee,
                            'rating' => $consultation->rating,
                            'review' => $consultation->review,
                            'created_at' => $consultation->created_at,
                            'started_at' => $consultation->started_at,
                            'ended_at' => $consultation->ended_at,
                        ];
                    }
                });

            return response()->json([
                'success' => true,
                'data' => $consultations,
                'role' => $isPsychologist ? 'psychologist' : 'user'
            ]);

        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
                'message' => 'Gagal mengambil data konsultasi',
                'error' => $e->getMessage()
            ], 500);
        }
    }
}
